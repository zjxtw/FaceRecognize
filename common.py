...
def drawbbox(image, bbox, color=None, thickness=2, textcolor=(0, 0, 0), landmarkcolor=(0, 0, 255)):
    ...
    # cv2.rectangle(image, (x - 50, y - 30, r - x + 1 + 100, b - y + 1 + 60), color, thickness, 16)

    image_h = image.shape[0]
    image_w = image.shape[1]
    return image[(y - 30) if (y - 30) > 0 else 0:(b + 30) if (b + 30) < image_h else image_h,
           (x - 50) if (x - 50) > 0 else 0:(r + 50) if (r + 50) < image_w else image_w]
